...
H5P.PythonTerminal = (function ($, Question) {
  ...
  /**
   * Decodificar entidades HTML en strings
   */
        ...
        if (keywordGroups.length > 0) {
          keywordGroups.forEach(function(group, groupIndex) {
            ...
            group.keywords.forEach(function(keyword) {
              ...
              if (!strictValidation) {
                keywordFound = code.indexOf(keyword) !== -1;
              } else {
                ...
                // Si es una función que requiere llamada, validar uso correcto
                  ...
                  keywordFound = self.isKeywordUsedAsFunction(code, keyword);
                }
                // Para otras palabras clave (métodos, variables, etc.), validar existencia
                else {
                  keywordFound = self.isValidWordOrNumber(code, keyword);
                }
              }
              
              if (keywordFound) {
                groupFound = true;
                foundInGroup.push(keyword);
              }
            });
