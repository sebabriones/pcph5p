  ...
  if (!elementParams) {
    ...
    if (library === 'GoToSlide') {
      ...
      switch (libraryName) {
        ...
        case 'H5P.PythonTerminal':
          elementParams.width = 90;
          elementParams.height = 85;
          break;
      }
    }
...
H5PEditor.CoursePresentation.prototype.addDNBButton = function (library, options) {
  ...
  var defaultTitle = library.title;
  if (id === 'pythonterminal') {
    defaultTitle = 'Python Terminal';
  }

  return {
    id: options.id || id,
    title: (options.title === undefined) ? defaultTitle : options.title,
