...
function C(options, contentId, contentData) {
  ...
  for (let posicion = 0; posicion < task.elements.length; posicion++) {
    shufflePos.push(posicion);
  }

  shufflePos.sort((a,b)=>Math.random()>=0.5?-1:1);
  ...
  for (let indice = 0; indice < task.elements.length; indice++) {
    shuffleElem.push({'ejex': task.elements[shufflePos[indice]].x, 'ejey': task.elements[shufflePos[indice]].y});
  }
