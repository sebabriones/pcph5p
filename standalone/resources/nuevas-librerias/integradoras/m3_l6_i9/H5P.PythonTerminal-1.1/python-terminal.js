...
H5P.PythonTerminal = (function ($, Question) {
  ...
  function PythonTerminal(params, contentId, contentData) {
    ...
    // Normalizar ejercicios requeridos para corregir problemas de guardado de Lumi
    // Esto asegura que keywords y keywordGroups siempre sean arrays, incluso si Lumi los guarda incorrectamente
    if (this.params.requiredExercises && Array.isArray(this.params.requiredExercises)) {
      var self = this;
      this.params.requiredExercises = this.params.requiredExercises.map(function(exercise) {
        return self.normalizeExerciseValidation(exercise);
      });
    }
  ...
  /**
   * Normalizar la estructura de validación de un ejercicio
   * Corrige problemas de guardado de Lumi donde keywords o keywordGroups pueden estar mal formados
   * @param {Object} exercise - Ejercicio a normalizar
   * @return {Object} Ejercicio normalizado
   */
  PythonTerminal.prototype.normalizeExerciseValidation = function(exercise) {
    if (!exercise) {
      return exercise;
    }
    
    // Asegurar que validation existe
    if (!exercise.validation) {
      exercise.validation = {};
    }
    
    // Asegurar que keywords siempre es un array
    if (!exercise.validation.keywords || !Array.isArray(exercise.validation.keywords)) {
      exercise.validation.keywords = [];
    }
    
    // Asegurar que keywordGroups siempre es un array
    if (!exercise.validation.keywordGroups || !Array.isArray(exercise.validation.keywordGroups)) {
      exercise.validation.keywordGroups = [];
    }
    
    // Asegurar que listValidation siempre es un array
    if (!exercise.validation.listValidation || !Array.isArray(exercise.validation.listValidation)) {
      exercise.validation.listValidation = [];
    }
    
    // Normalizar cada keywordGroup para asegurar que sus keywords también sean arrays
    if (exercise.validation.keywordGroups && exercise.validation.keywordGroups.length > 0) {
      exercise.validation.keywordGroups = exercise.validation.keywordGroups.map(function(group) {
    ...
    // Normalizar campos de tipo string que pueden estar como string vacío en lugar de undefined
    if (exercise.validation.inputType === '') {
      exercise.validation.inputType = undefined;
    }
    if (exercise.validation.outputType === '') {
      exercise.validation.outputType = undefined;
    }
    if (exercise.validation.inputStringFormat === '') {
      exercise.validation.inputStringFormat = undefined;
    }
    if (exercise.validation.outputStringFormat === '') {
      exercise.validation.outputStringFormat = undefined;
    }
    if (exercise.validation.inputStringCase === '') {
      exercise.validation.inputStringCase = undefined;
    }
    if (exercise.validation.outputStringCase === '') {
      exercise.validation.outputStringCase = undefined;
    }
  ...
  PythonTerminal.prototype.checkExerciseCompletion = function(code, success) {
    ...
    self.params.requiredExercises.forEach(function(exercise, index) {
      ...
      if (exercise && exercise.validation) {
        ...
        // Validación adicional: Si keywords está vacío y no hay keywordGroups ni listValidation,
        // el ejercicio debe tener al menos inputType o outputType para ser válido
        if (exercise.validation.keywords.length === 0 && 
            (!exercise.validation.keywordGroups || exercise.validation.keywordGroups.length === 0) &&
            (!exercise.validation.listValidation || exercise.validation.listValidation.length === 0)) {
          // Si no hay ningún tipo de validación configurado, el ejercicio no puede validarse
          if (!exercise.validation.inputType && !exercise.validation.outputType) {
            matches = false;
            missingKeywords.push('El ejercicio debe tener al menos keywords, keywordGroups, inputType, outputType o listValidation configurados');
          }
        }
        
        // Validar tipo de dato de entrada si está especificado
        // IMPORTANTE: Si inputType está definido, DEBE haber input() en el código
        if (exercise.validation.inputType && exercise.validation.inputType !== '') {
          // Verificar que input() esté siendo usado correctamente
          var hasInput = self.isKeywordUsedAsFunction(code, 'input');
          
          // Si inputType está definido pero no hay input(), fallar explícitamente
          if (!hasInput) {
            matches = false;
            missingKeywords.push('input() (requerido cuando se especifica inputType: ' + exercise.validation.inputType + ')');
          } else if (matches) {
            // Solo continuar con la validación si matches es true (puede haber fallado en keywords)
            ...
            } else if (exercise.validation.inputType === 'str') {
              // Validar formato del string de entrada (usar valores capturados en tiempo de ejecución)
              var inputFormat = exercise.validation.inputStringFormat || '';
              var inputCase = exercise.validation.inputStringCase || '';
              
              if (inputFormat !== '' || inputCase !== '') {
                // Calcular qué input corresponde a este ejercicio
                // Contar cuántos ejercicios anteriores tienen validación de input
                var inputIndex = 0;
                for (var j = 0; j < index; j++) {
                  var prevExercise = self.params.requiredExercises[j];
                  if (prevExercise && prevExercise.validation && 
                      (prevExercise.validation.inputType === 'str' || 
                       prevExercise.validation.inputStringCase || 
                       prevExercise.validation.inputStringFormat)) {
                    inputIndex++;
                  }
                }
                
                if (!self.validateStringsInCode(code, inputFormat, 'input', self.capturedInputs, null, inputCase, inputIndex)) {
                  matches = false;
                  var formatDescription = '';
                  if (inputFormat === 'letters_only') {
                    formatDescription = 'solo letras';
                  } else if (inputFormat === 'numbers_only') {
                    formatDescription = 'solo números';
                  } else if (inputFormat === 'letters_and_numbers') {
                    formatDescription = 'letras y números';
                  } else if (inputFormat === 'letters_and_special') {
                    formatDescription = 'letras y caracteres especiales';
                  }
                  
                  var caseDescription = '';
                  if (inputCase === 'upper') {
                    caseDescription = 'todas las letras en mayúscula';
                  } else if (inputCase === 'lower') {
                    caseDescription = 'todas las letras en minúscula';
                  } else if (inputCase === 'title') {
                    caseDescription = 'primera letra de cada palabra en mayúscula';
                  } else if (inputCase === 'capitalize') {
                    caseDescription = 'solo la primera letra en mayúscula';
                  }
                  
                  if (formatDescription !== '' && caseDescription !== '') {
                    missingKeywords.push('string de entrada debe contener ' + formatDescription + ' y tener ' + caseDescription);
                  } else if (formatDescription !== '') {
                    missingKeywords.push('string de entrada debe contener ' + formatDescription);
                  } else if (caseDescription !== '') {
                    missingKeywords.push('string de entrada debe tener ' + caseDescription);
                  }
                }
              }
            }
          }
          // Cierre del else if (matches) - si no hay input(), ya se marcó matches = false arriba
        }
        ...
        if (matches && exercise.validation.outputType && exercise.validation.outputType !== '') {
          if (exercise.validation.outputType === 'str') {
            ...
            var outputFormat = exercise.validation.outputStringFormat || '';
            var outputCase = exercise.validation.outputStringCase || '';
            
            if (outputFormat !== '' || outputCase !== '') {
              if (!self.validateStringsInCode(code, outputFormat, 'output', null, self.currentExecutionOutput, outputCase)) {
                matches = false;
                var formatDescription = '';
                if (outputFormat === 'letters_only') {
                  formatDescription = 'solo letras';
                } else if (outputFormat === 'numbers_only') {
                  formatDescription = 'solo números';
                } else if (outputFormat === 'letters_and_numbers') {
                  formatDescription = 'letras y números';
                } else if (outputFormat === 'letters_and_special') {
                  formatDescription = 'letras y caracteres especiales';
                }
                ...
                if (formatDescription !== '' && caseDescription !== '') {
                  missingKeywords.push('string de salida debe contener ' + formatDescription + ' y tener ' + caseDescription);
                } else if (formatDescription !== '') {
                  missingKeywords.push('string de salida debe contener ' + formatDescription);
                } else if (caseDescription !== '') {
                  missingKeywords.push('string de salida debe tener ' + caseDescription);
                }
              }
            }
          } else {
