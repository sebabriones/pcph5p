  ...
  function PipeFlow(options, id) {
    ...
    self.options = $.extend(true, {}, {
      ...
      messages: {
        ...
        leakMessage: 'El agua se escapa del circuito.',
        ...
        destNoWaterMessage: '@dest: el agua no llegó.',
        destWrongSourceMessage: '@dest: agua incorrecta (@sources).',
        destMissingSourceMessage: '@dest: falta agua de alguna fuente.'
      },
      settings: {
        animationSpeed: 100,
        showPathInfo: true,
  ...
  PipeFlow.prototype.animateWaterFlowNew = function () {
    ...
    sourceReachability.forEach(function (reachable, srcId) {
      ...
      reachable.forEach(function (distance, cellKey) {
        ...
        var canReachAnyDest = false;
        self.gameState.destinations.forEach(function (destPos) {
          if (canReachAnyDest) return;
          if (self.findPath({ row: r, col: c }, destPos).length > 0) {
            canReachAnyDest = true;
          }
        });
        if (!canReachAnyDest) {
