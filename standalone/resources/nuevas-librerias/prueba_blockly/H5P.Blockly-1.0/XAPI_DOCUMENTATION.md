...
|------|------|-------------|
| **score** | number | Puntuación basada en eficiencia (1 a maxScore) |
| **maxScore** | number | Igual al número óptimo de pasos configurado |

### Sistema de Puntuación

La puntuación se calcula basándose en la eficiencia de la solución:

**Fórmula:**
```javascript
if (blocksUsed <= optimalSteps) {
  score = maxScore;  // Puntuación perfecta
} else {
  score = Math.max(1, Math.round(maxScore * (optimalSteps / blocksUsed)));
}
```

**Ejemplos (con optimalSteps = 7):**
- Usa 7 bloques o menos: `score = 7` (100%)
- Usa 10 bloques: `score = 7 * (7/10) = 5` (71%)
- Usa 14 bloques: `score = 7 * (7/14) = 4` (57%)
- Usa 21 bloques: `score = 7 * (7/21) = 2` (29%)

**Estados:**
- Completa con solución óptima: `score = maxScore`
- Completa con más bloques: `score < maxScore` (proporcional)
- Falla: `score = 0`
...
**Ejemplo 1: Solución óptima (7 bloques usados, óptimo = 7)**
```json
{
  "verb": {
...
**Ejemplo 2: Solución no óptima (10 bloques usados, óptimo = 7)**
```json
{
  "verb": {
    "id": "http://adlnet.gov/expapi/verbs/completed"
