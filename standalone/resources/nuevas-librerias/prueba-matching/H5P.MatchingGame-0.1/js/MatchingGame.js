...
H5P.MatchingGame = (function ($, Question) {
    ...
    MatchingGame.prototype.createGameContent = function () {
        ...
        $leftColumn.html(self.options.pairs.map((pair, i) =>
            `<div class="item term" data-id="${pair.idAns}" data-name="${pair.answer}">${pair.answer}</div>`
        ).join(''));
        
        $rightColumn.html(self.definitions.map((pair, i) =>
            `<div class="item definition" data-id="${pair.idDef}" data-name="${pair.answer}">${pair.definition}</div>`
        ).join(''));
    ...
    //Redimensiona las líneas que unen los terminos y conceptos
    MatchingGame.prototype.listener = function () {
        window.addEventListener('resize', ()=>{
            let self = this;

            if(typeof self.matchesId !== 'undefined'){
                $('.lines-svg').html('');

                self.matchesId.forEach(match => {
                    self.drawConnection($(`div[data-id=${match[0]}]`)[0], $(`div[data-id=${match[1]}]`)[0]);
                });
            }
        });
    }
